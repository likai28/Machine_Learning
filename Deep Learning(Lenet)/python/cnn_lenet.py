...
def pooling_layer_forward(input, layer):
  ...
  output_n = np.zeros((h_out,w_out,c))
  for n in range(batch_size):
    input_n = input['data'][:,n]
    input_n = np.reshape(input_n,(h_in,w_in,c))
    for i in range(c):
      tmp = input_n[:,:,i]
      for h in range(h_out):
        for w in range(w_out):
          output_n[h,w,i] = np.amax(tmp[h*stride: h*stride+k, w*stride: w*stride+k])
    output['data'][:,n] = np.reshape(output_n,(1,(h_out * w_out * c)))
...
def inner_product_forward(input, layer, param):
  ...
  # implementation begins
  output['data'] = input['data'].T.dot(param['w'])+param['b']
